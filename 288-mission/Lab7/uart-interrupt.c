...
volatile char scan_byte = 's'; // byte value for special character used as a command
volatile char move_byte = 'w'; // byte value for special character used as a command
volatile char rotate_byte = 'e'; // byte value for special character used as a command
volatile char auto_byte = 'a'; // byte value for special character used as a command
volatile char terminate_byte = 't'; // byte value for special character used as a command
volatile char command_flag = ""; // flag to tell the main program a special command was received
...
void uart_interrupt_init(void){
  ...
  uint16_t fBRD = (int)(0.6806 * 64 + 0.5); //use equations
...
void UART1_Handler(void)
{
    ...
    if (UART1_MIS_R & 0x00000010)
    {
        ...
        byte_received = uart_receive;//_nonblocking();
        ...
        else
        {
            ...
            /*if (byte_received == scan_byte)
            {
                command_flag = 1;
            }

            if (byte_received == move_byte)
            {
                command_flag = 2;
            }

            if (byte_received == rotate_byte)
            {
                command_flag = 3;
            }

            if (byte_received == auto_byte)
            {
                command_flag = 4;
            }

            if (byte_received == terminate_byte)
            {
                command_flag = 5;
            }

            if (command_flag == 0){
                command_flag = byte_received;
            }*/
