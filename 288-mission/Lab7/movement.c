...
float move_forward (oi_t *sensor_data, float distance_cm){
    //lcd_init();

    float distance_mm = 10 * distance_cm;
    ...
    while (sum < distance_mm) {
        ...
        if(sensor_data->cliffFrontLeft || sensor_data->cliffFrontRight || sensor_data->cliffRight || sensor_data->cliffLeft){
            oi_setWheels(0,0);
            gui_add(4, 90, 5, 10);
            oi_setWheels(0,0);
...
float move_backward (oi_t *sensor_data, float distance_cm){
    //lcd_init();
