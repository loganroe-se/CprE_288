...
void find_objects(oi_t *sensor_data, int min, int max, int interval) {
    found_object** objects = malloc(20 * sizeof(found_object*));
    int o_alloc;
    for (o_alloc = 0; o_alloc < 20; o_alloc++) {
        objects[o_alloc] = calloc(1, sizeof(found_object));
    }
